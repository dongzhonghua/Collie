...
import xyz.dsvshx.peony.MyClassLoader;
        ...
        try {
            ...
            for (CtMethod ctMethod : declaredMethods) {
                ...
                ctMethod.insertAfter(
                        "{xyz.dsvshx.peony.instrumentation.Point.point(\"" + methodId + "\", $args, startNanos, System"
                                + ".nanoTime(), $_);};");
                // 添加异常捕获，打点之后再throw
                ...
                ctMethod.addCatch(
                        "{xyz.dsvshx.peony.instrumentation.Point.point(\"" + methodId
                                + "\", $args, -1L, System.nanoTime(), $e);"
                                + " throw $e; };", ClassPool.getDefault().get("java.lang.Throwable"));

            }
            MyClassLoader myClassLoader = new MyClassLoader(
                    "/Users/dongzhonghua03/Documents/github/peony/peony-test/src/main/java/xyz/dsvshx/peony");
            // 加载之后的类，这里需要用一个别的类加载器，否则会报重复的错误，因为一个类加载器只能加载一个名字相同的类。
            Class<?> aClass = ctClass.toClass(myClassLoader);
            //获取修改之后的class文件
            ctClass.writeFile("/Users/dongzhonghua03/Documents/github/peony/peony-test/src/test/java");
            Object obj = aClass.newInstance();
            myClassLoader.loadClass("xyz.dsvshx.peony.agent.instrumentation.Point");
